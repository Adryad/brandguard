...
class LegalNewsCollector:
    ...
    ALLOWED_DOMAINS = [
        ...
        'cnbc.com', 'marketwatch.com', 'forbes.com', 'bloomberg.com',
        'apnews.com', 'upi.com', 'afp.com'
    ]
    ...
    async def collect_company_news(self, company: Company, days_back: int = 30) -> List[Dict]:
        """
        ...
        for source in sources:
            try:
                ...
                articles.extend(await self._fetch_from_source(source, company_keywords, days_back))
    ...
    async def _fetch_from_source(self, source: DataSource, keywords: List[str], days_back: int) -> List[Dict]:
        """
    ...
    async def _fetch_rss_feed(self, rss_url: str, keywords: List[str], days_back: int) -> List[Dict]:
        """
    ...
    def _parse_rss_content(self, content: str, keywords: List[str], days_back: int) -> List[Dict]:
        """
    ...
    def _store_article(self, company_id: int, source_id: int, article_data: Dict) -> Article:
        """
        ...
        article = Article(
            ...
            is_public=True,
            data_retention_until=datetime.utcnow() + timedelta(days=settings.DATA_RETENTION_DAYS)
        )
