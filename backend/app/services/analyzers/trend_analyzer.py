...
class TrendAnalyzer:
    ...
    def _calculate_trend(self, data_points: List[Dict]) -> Dict:
        ...
        # Determine trend
        recent_ma = df['ma_7'].iloc[-1] if not pd.isna(df['ma_7'].iloc[-1]) else df['sentiment_score'].iloc[-1]
    ...
    def generate_trend_summary(self, trends_data: Dict) -> str:
        ...
        if trend == 'improving':
            ...
            summary = f"Reputation is declining with a momentum of {abs(momentum):.2%}. "
        else:
        ...
        if trends_data['predictions']:
            avg_prediction = np.mean([p['predicted_score'] for p in trends_data['predictions']])
